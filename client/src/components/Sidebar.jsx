        ...
        filteredUsers.map((user,i) => {
            return (
                <div onClick={() => {setSelectedUser(user);setUnseenMessages(prev => (
                  {...prev,[user._id] : 0}
                ))}} key={i} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
                      ...
                      {
                        unseenMessages[user._id] > 0 && <p className='absolute top-4 right-4 p-[2px] text-xs gap-5 h-5 w-5 flex justify-center rounded-full bg-violet-500/50'>{unseenMessages[user._id]}</p>
                      } 
                </div>
            )
        })
